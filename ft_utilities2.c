...
/*   Updated: 2018/05/06 20:42:19 by lprior           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_check_file(t_env *all, struct stat *info)
{
	...
	if (lstat(all->paths[all->px], info) || (!S_ISDIR(info->st_mode)
	...
	&& ft_strcmp(all->paths[all->px], ".")))
	{
		ft_error(1, all->paths[all->px]);
		all->paths[all->px] = NULL;
	}
	else if (all->paths[all->px] && ft_strcmp(all->paths[all->px], ".")
		&& !S_ISDIR(info->st_mode))
		all->args = ft_create_node(all->args, all->paths[all->px], PATH);
	else if (all->paths[all->px] && (!ft_strcmp(all->paths[all->px], ".")
		|| S_ISDIR(info->st_mode)))
		all->dargs = ft_create_node(all->dargs, all->paths[all->px], PATH);
	else
